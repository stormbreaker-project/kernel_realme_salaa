...
static struct wakeup_source ffd_wake_lock;
#else
static struct wakeup_source *ffd_wake_lock = NULL;
#endif
...
static int camera_protect_flush_report(void)
{
...
static int camera_protect_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    if (event->flush_action == DATA_ACTION) {
        ...
        #if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0))
        __pm_wakeup_event(&ffd_wake_lock, msecs_to_jiffies(100));
        #else
        __pm_wakeup_event(ffd_wake_lock, msecs_to_jiffies(100));
        #endif
...
static int camera_protecthub_local_init(void)
{
    ...
    #if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0))
    wakeup_source_init(&ffd_wake_lock, "ffd_wake_lock");
    #else
    ffd_wake_lock = wakeup_source_register(NULL,"ffd_wake_lock");
    #endif
    return 0;
exit:
    return -1;
